...
pub(crate) fn call_method<'p>(interp: &mut Interp<'p>, recv: Value<'p>, name: &str, args: Args<'p>) -> R<'p> {
    ...
    let result = match &recv {
        ...
        Value::Record(r) if &*r.ty == REQUEST && name == "json" => response_method(&r.fields, name),
        Value::Record(r) if &*r.ty == DATABASE => Some(database_method(interp, &r.fields, name, args)),
        Value::Record(r) if &*r.ty == crate::eval::conversation::CONVERSATION => {
            Some(interp.conversation_method(&r.fields, name, &args))
        }
