...
pub(crate) struct Shared<'p> {
    ...
    /// The application's database (`database "…"`, a test's own in tests).
    pub app_db: Mutex<Option<Value<'p>>>,
    /// Declared `migration`s, in order.
    pub migrations: Mutex<Vec<crate::eval::records::Migration<'p>>>,
